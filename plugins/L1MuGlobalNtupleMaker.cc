...
L1MuGlobalNtupleMaker::L1MuGlobalNtupleMaker(const edm::ParameterSet& iConfig) :
  ...
  _maxTkMuonStubsBMTF(iConfig.getParameter<int>("maxTkMuonStubs")),
  _maxTkMuonStubsEMTF(iConfig.getParameter<int>("maxTkMuonStubs")),
  _maxTkMuonStubsOMTF(iConfig.getParameter<int>("maxTkMuonStubs")),
  ...
  //_TkMuonStubsToken(consumes<l1t::L1TkMuonParticleCollection>(iConfig.getParameter<edm::InputTag>("tkMuonStubs"))),
...
void L1MuGlobalNtupleMaker::create_trees()
{
  ...
  //TkMuStubs muons
  _mytree->Branch("tkmuStubs_pt",&_tkmuStubs_pt);
  _mytree->Branch("tkmuStubs_eta",&_tkmuStubs_eta);
  _mytree->Branch("tkmuStubs_phi",&_tkmuStubs_phi);
  _mytree->Branch("tkmuStubs_charge",&_tkmuStubs_charge);
  _mytree->Branch("tkmuStubs_tkiso",&_tkmuStubs_tkiso);
  _mytree->Branch("tkmuStubs_Vz", &_tkmuStubs_Vz);
  //_mytree->Branch("tkmuStubs_charge", &_tkmuStubs_charge);

  _mytree->Branch("tkmuStubs_Nmuons",&_tkmuStubs_Nmuons);
  ...
  //_mytree->Branch("tttracks_d0",&_tttracks_d0);
  _mytree->Branch("tttracks_z0",&_tttracks_z0);
  _mytree->Branch("tttracks_bendchi2",&_tttracks_bendchi2);
  //_mytree->Branch("tttracks_nstubs",&_tttracks_nstubs);
  //_mytree->Branch("tttracks_lhits",&_tttracks_lhits);
  //_mytree->Branch("tttracks_seeds",&_tttracks_seeds);
  //_mytree->Branch("tttracks_genuine",&_tttracks_genuine);
  //_mytree->Branch("tttracks_loose",&_tttracks_loose);
...
void L1MuGlobalNtupleMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  //iEvent.getByToken(_TkMuonStubsToken, TkMuonStubs);
  ...
  if (TkMuonStubsBMTF.isValid()){
      ...
      SetTkMuonStubs(TkMuonStubsBMTF, _maxTkMuonStubsBMTF,1);
  } else {
  ...
  if (TkMuonStubsOMTF.isValid()){
      SetTkMuonStubsOMTF(TkMuonStubsOMTF, _maxTkMuonStubsOMTF, 2);
      //SetTkMuonStubs(TkMuonStubsOMTF, _maxTkMuonStubsOMTF, 2);
  ...
  if (TkMuonStubsEMTF.isValid()){
      SetTkMuonStubsEMTF(TkMuonStubsEMTF, _maxTkMuonStubsEMTF, 3);
      SetTkMuonStubs(TkMuonStubsEMTF, _maxTkMuonStubsEMTF, 3);
    } else{
  ...
  cout <<"The output of Tttrack is Valid is: " << Tttrack.isValid() << endl; 
  if(Tttrack.isValid()){
  ...
  //  SetTTTracks(TTTrackHandle, _maxTTTracks);
  // } else{
  //   edm::LogWarning("MissingProduct") << "L1 Phase2 Tttrack not found. Branch will not be filled" << std::endl;
  //}
...
void L1MuGlobalNtupleMaker::SetTkMuons(const edm::Handle<l1t::L1TkMuonParticleCollection> muon, int maxTkMuons)
{
  ...
  for (l1t::L1TkMuonParticleCollection::const_iterator it=muon->begin(); it!=muon->end() && _tkmu_Nmuons < maxTkMuons; it++){
      //cout << "***!!! the max muon num is" << _tkmu_Nmuons <<   endl;
      //cout << "***!!! The first it is " << it->eta() <<   endl;
...
void L1MuGlobalNtupleMaker::SetTkMuonStubsBMTF(const edm::Handle<l1t::L1TkMuonParticleCollection> muon, int maxTkMuonStubs, unsigned int muonDetector)
{
      cout << "****** The max tkMuStub number is " << maxTkMuonStubs << endl;
  ...
  for(l1t::L1TkMuonParticleCollection::const_iterator it=muon->begin(); it!=muon->end() && _tkmuStubsBMTF_Nmuons < maxTkMuonStubs; it++){
...
void L1MuGlobalNtupleMaker::SetTkMuonStubsEMTF(const edm::Handle<l1t::L1TkMuonParticleCollection> muon, int maxTkMuonStubs, unsigned int muonDetector)
{
  ...
  for(l1t::L1TkMuonParticleCollection::const_iterator it=muon->begin(); it!=muon->end() && _tkmuStubsEMTF_Nmuons < maxTkMuonStubs; it++){
    //if (muonDetector==1 && fabs(it->eta())>=0.9) continue;
    //if (muonDetector==3 && fabs(it->eta())<1.2) continue;
...
void L1MuGlobalNtupleMaker::SetTkMuonStubsOMTF(const edm::Handle<l1t::BayesMuCorrTrackBxCollection> muon, int maxTkMuonStubs, unsigned int muonDetector)
{
  ...
  for (int ibx = muon->getFirstBX(); ibx <= muon->getLastBX(); ++ibx) {
  for(l1t::BayesMuCorrTrackBxCollection::const_iterator it=muon->begin(); it!=muon->end() && _tkmuStubsOMTF_Nmuons < maxTkMuonStubs; it++){
    //if (muonDetector==1 && fabs(it->eta())>=0.9) continue;
    //if (muonDetector==3 && fabs(it->eta())<1.2) continue;
...
void L1MuGlobalNtupleMaker::SetTkMuonStubs(const edm::Handle<l1t::L1TkMuonParticleCollection> muon, int maxTkMuonStubs, unsigned int muonDetector)
{
      _tkmuStubs_pt.clear();
      _tkmuStubs_eta.clear();
      _tkmuStubs_phi.clear();
      _tkmuStubs_charge.clear();
      _tkmuStubs_tkiso.clear();
      _tkmuStubs_Vz.clear();
      _tkmuStubs_Nmuons=0;

  for(l1t::L1TkMuonParticleCollection::const_iterator it=muon->begin(); it!=muon->end() && _tkmuStubs_Nmuons < maxTkMuonStubs; it++){
    //if (muonDetector==1 && fabs(it->eta())>=0.9) continue;
    //if (muonDetector==3 && fabs(it->eta())<1.2) continue;

    if (it->pt() > 0){
      _tkmuStubs_pt.push_back(it->pt());
      _tkmuStubs_eta.push_back(it->eta());
      _tkmuStubs_phi.push_back(it->phi());
      _tkmuStubs_charge.push_back(it->charge());
      _tkmuStubs_tkiso.push_back(it->getTrkIsol());
      _tkmuStubs_Vz.push_back(it->getTrkzVtx());
